...
<td>
    Нативные корутины. Используются совместно с `await`. Часть языка, а не какой-либо библиотеки. Хорошо оптимизированы и рекомендуются к использованию.
...
и множество подобных проектов, например, nodejs, браузеры, nginx, qt, gtk, netty ([NioEventLoop.java](https://github.com/netty/netty/blob/4.1/transport/src/main/java/io/netty/channel/nio/NioEventLoop.java#L437-L537)), 
...
Современная асинхронная разработка в питоне базируется на [event loop'е](#event_loop) и [корутинах](#coroutines).  
...
В питоне существует три типа awaitable-объектов (имеют метод `__await__`):

* корутины (для выполнения на event loop'е преобразуется в Task/Future)
* Futures (код [base_futures.py](https://github.com/python/cpython/blob/3.9/Lib/asyncio/base_futures.py), [futures](https://github.com/python/cpython/blob/3.9/Lib/asyncio/futures.py#L29)). 
...
* Tasks (наследуются от Future, код [base_tasks.py](https://github.com/python/cpython/blob/3.9/Lib/asyncio/base_tasks.py), [tasks.py](https://github.com/python/cpython/blob/3.9/Lib/asyncio/tasks.py#L239-L324)).
